...
export const Inventory: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const lowStock = mockInventory.filter(i => i.stock > 0 && i.stock <= i.lowStockThreshold).length;
    ...
    const fastMoving = sortedBySales.slice(0, 5).map(i => ({ name: i.name, value: i.sales, type: 'Fast' }));
    const slowMoving = sortedBySales.slice(-5).reverse().map(i => ({ name: i.name, value: i.sales, type: 'Slow' }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
            <CardHeader>
                ...
                <CardDescription>Aggregate inventory levels over the last 14 days.</CardDescription>
            </CardHeader>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>Fastest vs. Slowest moving products by sales volume.</CardDescription>
            </CardHeader>
            <CardContent className="pl-2">
                <div className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={stats.velocityData} layout="vertical" margin={{ left: 40 }}>
                             ...
                             <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                                {stats.velocityData.map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={entry.type === 'Fast' ? 'hsl(var(--primary))' : 'hsl(var(--muted))'} />
                                ))}
                             </Bar>
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription>Real-time stock levels and sales performance.</CardDescription>
        </CardHeader>
            ...
            <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y">
                    {mockInventory.map(item => {
                        const stockStatus = item.stock === 0 ? 'Out of Stock' : item.stock <= item.lowStockThreshold ? 'Low Stock' : 'In Stock';
                        const badgeVariant = stockStatus === 'Out of Stock' ? 'destructive' : stockStatus === 'Low Stock' ? 'warning' : 'success';
                            ...
                            <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center">
                                        <img src={item.image} alt="" className="h-8 w-8 rounded border object-cover mr-3"/>
                                ...
                                <td className="px-6 py-4 text-right font-medium">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <div className="h-1.5 w-16 bg-secondary rounded-full overflow-hidden">
                                            <div 
                                                className={`h-full rounded-full ${stockStatus === 'Low Stock' ? 'bg-yellow-500' : stockStatus === 'Out of Stock' ? 'bg-red-500' : 'bg-green-500'}`} 
